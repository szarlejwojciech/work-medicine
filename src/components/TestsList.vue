<template>
  <v-card>
    <v-list class="pb-5">
      ...
      <v-list-group class="px-5" v-for="(group, i) in dicplayTests" :key="i" :value="true" prepend-icon="mdi-account-circle" dense no-action>
  ...
  setup(props) {
    ...
    const dicplayTests = computed(() =>
      examinationsList.items
        .map((item) => ({
          ...item,
          arrayValues: item.arrayValues.filter(({ text }) => tests.value.includes(text)),
        }))
